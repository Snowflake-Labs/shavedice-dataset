...
We start by restricting to a 2-week subset of the data from Summer 2024, aggregate the different regions and SKUs together to a single timeseries of total VM demand, and normalize this to a 100 unit peak over that time window.

```{r}
data <- read_parquet("../hourly_normalized.parquet")
data <- data %>% subset(REGION_NUM == 2) %>%
  subset(USAGE_HOUR >= as.POSIXct("2024-06-02", tz="UTC")) %>%
  subset(USAGE_HOUR < as.POSIXct("2024-06-16", tz="UTC"))
dim(data)
head(data)
